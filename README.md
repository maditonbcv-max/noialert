...
- **緊急警報16種 + 定型連絡6種** … VOICEVOX事前生成のOpus固定音声（即時・割り込み制御つき）
- **自由入力TTS** … OpenAI APIのみランタイム生成
...
```bash
npm run gen:audio            # 全22ファイルを audio/ へ生成
...
起動するとログchに「警報Bot起動完了。VC接続済み。音声22件をプリロードしました。」が投稿される。
...
2. ローカル確認:
   ```bash
   npx serve web   # 任意の静的サーバーで web/ を配信
   ```
...
- `web/config.js` の `apiBase` を本番APIのURLにしてからデプロイ。
...
定型TTS6: `tts_shugo_5min` `tts_bouei_keizoku` `tts_hokyu` `tts_teisatsu` `tts_migi` `tts_hidari`

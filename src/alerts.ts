...
export const FREE_TTS_PER_SENDER_MS = 0; // 0 = 自由TTSの発信者単位制限なし(IP単位60/分の保険は残る)
export const CHAT_TTS_THROTTLE_MS = 3_000; // チャット読み上げの同一ユーザー連投制限

// interrupt は既定で優先度92以上。第5引数で個別に上書きできる。
const fixed = (
  id: string, label: string, priority: number, throttleMs: number,
  interrupt: boolean = priority >= INTERRUPT_THRESHOLD,
): AlertDef => ({
...
const FIXED: AlertDef[] = [
  ...
  fixed('onmitsu_honjin',  '敵隠密・本陣恐れ',                80, 10_000, true), // 割り込み(運用要望)
  ...
  fixed('awase_chuo_90s',      '中央 攻撃合わせ準備90秒',      64, 10_000),
  fixed('awase_tekihonjin_90s','敵本陣 攻撃合わせ準備90秒',    64, 10_000),
